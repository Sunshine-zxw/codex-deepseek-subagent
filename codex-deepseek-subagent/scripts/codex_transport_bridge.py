...
Codex continues speaking the Responses wire API. Only providers explicitly
configured as ``chat_completions_bridge`` are translated to an OpenAI-compatible
``/chat/completions`` upstream. The bridge preserves tool-call/tool-result
history and DeepSeek ``reasoning_content`` without adding Node.js or LiteLLM.
"""
...
DEFAULT_PORT = 48671
STATE_DIR_NAME = "codex-deepseek-subagent"
REGISTRY_NAME = "registry.json"
TRANSPORT_STATE_NAME = "transport-state.json"
PID_NAME = "transport-bridge.pid"
LOG_NAME = "transport-bridge.log"
...
def state_dir(codex_home: str | None = None) -> Path:
    return resolve_home(codex_home) / STATE_DIR_NAME


def registry_path(codex_home: str | None = None) -> Path:
    return state_dir(codex_home) / REGISTRY_NAME


def transport_state_path(codex_home: str | None = None) -> Path:
    return state_dir(codex_home) / TRANSPORT_STATE_NAME


def pid_path(codex_home: str | None = None) -> Path:
    return state_dir(codex_home) / PID_NAME


def log_path(codex_home: str | None = None) -> Path:
    return state_dir(codex_home) / LOG_NAME


def bridge_port() -> int:
    raw = os.environ.get("CODEX_SUBAGENT_BRIDGE_PORT")
    if raw is None:
        return DEFAULT_PORT
    try:
        port = int(raw)
...
def provider_base_url(provider_id: str, port: int | None = None) -> str:
    if not PROVIDER_RE.fullmatch(provider_id):
...
def _load_transport_state(codex_home: str | None = None) -> dict[str, Any]:
    ...
    try:
        ...
        raise BridgeError(
            500,
            f"transport state 无法读取：{path}",
            "transport_state_invalid",
        ) from exc


def effective_request_profile(configured: str) -> str:
    """Resolve auto conservatively.

    A reseller model named DeepSeek is not proof that the reseller accepts
    DeepSeek-native ``thinking`` parameters. Therefore ``auto`` means the plain
    OpenAI-compatible profile. Callers can opt into ``auto-tool-choice`` or a
    DeepSeek-specific profile explicitly after a compatibility test.
    """

    if configured not in REQUEST_PROFILES:
        raise BridgeError(500, f"未知 request_profile：{configured}", "request_profile_invalid")
...
def provider_for_request(
    ...
    transport_state = _load_transport_state(codex_home)
    ...
    provider_state = (transport_state.get("providers") or {}).get(provider_id) or {}
    if provider_state.get("transport", "responses") != TRANSPORT:
        raise BridgeError(
            400,
            f"Provider {provider_id} 未配置 {TRANSPORT}。",
            "transport_mismatch",
        )

    agent_state = transport_state.get("agents") or {}
    ...
    for role, agent in agents.items():
        ...
        if agent.get("provider") != provider_id or agent.get("model") != model:
            continue
        matched_roles.append(str(role))
        profile = (agent_state.get(role) or {}).get("request_profile", "auto")
        profiles.add(effective_request_profile(str(profile)))

    if not matched_roles:
        raise BridgeError(
            404,
            f"模型 {model} 未注册为 Provider {provider_id} 的子 Agent。",
            "model_not_registered",
        )
    if len(profiles) != 1:
        raise BridgeError(
            409,
            f"Provider {provider_id} / model {model} 的 Agent request_profile 冲突。",
            "request_profile_conflict",
        )
    return provider, next(iter(profiles))
...
def translate_tools(
    ...
    for tool in tools:
        ...
        name = str(tool.get("name") or nested.get("name") or "tool")
...
def _chat_name_for_original(
    ...
    for chat_name, metadata in mapping.items():
...
def coalesce_assistant_messages(
...
def ensure_tool_result_order(
    messages: list[dict[str, Any]],
) -> list[dict[str, Any]]:
    """Keep only tool rows that immediately follow the matching tool_calls."""
    ...
    while index < len(messages):
        ...
        for call_id in call_ids:
            if call_id in tool_rows:
                repaired.append(tool_rows[call_id])
    return repaired
...
def _translate_tool_choice(
    ...
    if kind in {"function", "custom"} and isinstance(name, str):
        return {
            "type": "function",
            "function": {"name": _chat_name_for_original(name, namespace, mapping)},
        }
...
def responses_to_chat(
    ...
    profile = effective_request_profile(request_profile)
    chat_tools, mapping = translate_tools(payload.get("tools"))

    messages: list[dict[str, Any]] = []
    instructions = payload.get("instructions")
    if isinstance(instructions, str) and instructions:
        messages.append({"role": "system", "content": instructions})
    messages.extend(translate_input(payload.get("input"), mapping))

    chat: dict[str, Any] = {
        "model": payload.get("model"),
        "messages": messages,
        # A complete upstream object keeps the bridge small and deterministic.
        # It is converted back into a valid Responses SSE stream for Codex.
        "stream": False,
    }
    if chat_tools:
        chat["tools"] = chat_tools
    tool_choice = _translate_tool_choice(payload.get("tool_choice"), mapping)
    ...
    if profile == "deepseek-thinking":
        ...
        # Plain OpenAI-compatible providers may understand reasoning_effort;
        # unknown providers that reject it will be surfaced by tool-test.
...
def chat_message_to_items(
    ...
    for call in message.get("tool_calls") or []:
        ...
        metadata = mapping.get(
            chat_name,
            {"kind": "function", "name": chat_name, "namespace": None},
        )
        ...
        if metadata.get("kind") == "custom":
            ...
            item: dict[str, Any] = {
                ...
                "call_id": call_id,
                "name": metadata.get("name") or chat_name,
...
def chat_to_responses(
    ...
    if not isinstance(choices, list) or not choices or not isinstance(choices[0], dict):
        raise BridgeError(
            502,
            "Chat Completions 响应缺少 choices[0]。",
            "upstream_invalid",
        )
    ...
    return {
        ...
        "output": chat_message_to_items(message, mapping),
        "usage": _usage_payload(chat_payload.get("usage")),
    }


def response_sse_events(response: dict[str, Any]) -> list[dict[str, Any]]:
    """Create the minimal Responses SSE sequence Codex consumes.

    The upstream call is deliberately non-streaming. Sending complete
    ``output_item.done`` records avoids pretending token-level streaming while
    still exercising Codex's Responses SSE parser.
    """
...
def call_upstream(
    ...
    try:
        ...
        raise BridgeError(
            exc.code,
            message or f"上游 HTTP {exc.code}",
            "upstream_http_error",
        ) from exc
    except urllib.error.URLError as exc:
        raise BridgeError(
            502,
            f"无法连接 Chat Completions 上游：{exc.reason}",
    ...
    try:
        ...
        raise BridgeError(
            502,
            "Chat Completions 上游返回了非 JSON 响应。",
            "upstream_invalid",
        ) from exc
    if not isinstance(payload, dict):
...
def translate_request(
    ...
    provider, profile = provider_for_request(provider_id, model, codex_home)
    chat_payload, mapping = responses_to_chat(payload, profile)
    upstream = call_upstream(provider, chat_payload, authorization)
    response = chat_to_responses(upstream, mapping)
...
class BridgeHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        if self.path.rstrip("/") == "/health":
            self._json(
                ...
                {
                    "ok": True,
                    "service": "codex-subagent-transport-bridge",
    ...
    def do_POST(self) -> None:  # noqa: N802
        ...
        match = re.fullmatch(r"/providers/([^/]+)/responses/?", path)
        if not match:
        ...
        if length <= 0 or length > MAX_BODY_BYTES:
            self._json(
                ...
                {"error": {"message": "invalid body length"}},
            )
            return
...
def serve(
    ...
    state_dir(str(home)).mkdir(parents=True, exist_ok=True)
...
def is_healthy(
    ...
    *,
    ...
    payload = health_payload(port, timeout=timeout)
    if not payload or payload.get("ok") is not True:
        return False
    if payload.get("version") != BRIDGE_VERSION:
        return False
    if codex_home is not None:
        return payload.get("codex_home") == str(resolve_home(codex_home))
    return True
...
def ensure_running(
    ...
    *,
    ...
    home = resolve_home(codex_home)
    port = bridge_port()
    existing = health_payload(port)
    if existing is not None:
        if (
            existing.get("ok") is True
            and existing.get("version") == BRIDGE_VERSION
            and existing.get("codex_home") == str(home)
        ):
            return {
                "running": True,
                "started": False,
        ...
        raise BridgeError(
            503,
            (
                f"端口 {port} 已被另一个 bridge/服务占用；"
                f"当前 health={existing}。请停止旧 bridge 或设置 CODEX_SUBAGENT_BRIDGE_PORT。"
            ),
    ...
    while time.time() < deadline:
        if is_healthy(port, codex_home=str(home)):
            return {
                "running": True,
                "started": True,
                "port": port,
    ...
    raise BridgeError(
        503,
        f"transport bridge 启动失败；查看日志：{log}",
        "bridge_start_failed",
    )
...
def stop(codex_home: str | None = None) -> dict[str, Any]:
    home = resolve_home(codex_home)
    path = pid_path(str(home))
    payload = health_payload()
    if payload and payload.get("codex_home") != str(home):
        raise BridgeError(
            409,
            "当前端口上的 bridge 属于另一个 CODEX_HOME，不会终止它。",
            "bridge_home_mismatch",
        )
    try:
        pid = int(path.read_text(encoding="utf-8").strip())
    except (FileNotFoundError, OSError, ValueError):
        return {
            "running": is_healthy(codex_home=str(home)),
            "stopped": False,
            "codex_home": str(home),
        }
    ...
    while time.time() < deadline and is_healthy(codex_home=str(home)):
        time.sleep(0.1)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    serve_parser.add_argument("--codex-home")
